...
<!-- class: lead -->
<!-- これから、チャプター6 セキュリティオペレーションについて説明します。 -->
...
<!-- header: "" -->
<!-- class: contents -->
<!-- ここでは、以下の流れに沿って説明していきます。SOCやSOCの業務内容に加えて、SOCが使用するツールや技術についても説明します。 -->
...
<!-- 
■ NOCとの違いについて
似たものとしてNOC(Network Operation Center)がありますが、そちらでは、通信ネットワークの管理・運用が主な業務となっていて、ネットワークを構成する回線や機器の監視や制御、トラブル対応などを一元的に行う。(一般的には、ISPなどが地域ごとに設ける集中的なネットワーク運用拠点のことを指すことが多い。) 
-->
...
<!-- (最初に)SOCには、次の2種類がある。プライベートSOCと、パブリックSOC/共有SOC。 -->
...
<!-- SOCの業務について図式化したものをここに示す。IOCやSIEMについては後のスライドで解説しますが、イメージとしては、NWから収集したログをインターネット上にあるセキュリティに関する情報と照合しながら、インシデントを発見したらすぐさまCSIRTへ連絡するような流れになっている。 -->
<!-- EPPについては、EDRの解説スライドにて説明する。 -->
...
<!-- SOCメンバーが利用するツールや情報について(、それぞれの役割や使われ方を)紹介します。 -->
<!-- 
● ルールでは、SIEM が通常の動作と悪意のある動作を区別する基準を定義する。例えば、メールにZIPファイルが添付されている場合など。複数のルールを組み合わせ、より複雑な行動パターンを識別できる。 
● FQDN(Fully Qualified Domain Name, 完全修飾ドメイン名)とは、サブドメインやホスト名などを省略せずに書いたドメイン名。
-->
...
■ IOCデータベース: XMLベースのOpenIOC(Mandiant社が作成し、2014年にFireEye社によって買収された)
■ TPP: TPPsともいわれ、代表例として、MITRE(米国の非営利団体)が提供する ATT&CK(Adversarial Tactics, Techniques and Commom Knowledge)というサイバーセキュリティフレームワークがある。攻撃者のTTPに関する情報を整理・集積しており、セキュリティ製品はこのTPPに基づいて検知・対応の仕組みを構築することが一般的。
-->
...
<!-- 自分の役割で出来ない場合、違う人に後回しにする -->
<!-- 体制の階層としては下から、Tier1、Tier2、そして、セキュリティアナリストが一番上に位置しています。 -->
...
<!-- header: "" -->
<!-- ここからは、一般的なSOCの監視業務について詳しく見ていきます。 -->
...
<!-- 最近では、機械学習を使った検知もあります。例えば、LSTM～～の異常検知手法などが研究されています。 -->
...
<!-- パッチ適用済みで脆弱性が修正済みの場合は、脆弱性を狙った攻撃は失敗しているため、誤検知と判定する。 -->
<!-- アノマリ検知、振る舞い検知の場合、ユーザの操作を検知したときは、権限の有無、操作の意図～～被害の発生の有無などを調査する。こうして、総合的に検知/誤検知やインシデントの有無を判断する -->
...
<!-- 不正侵入では、攻撃者が侵入したマシン数やネットワーク範囲、～～ -->
<!-- リソースの不正利用では、スパムメールの送信やサイバー攻撃をするときの踏み台、暗号通貨のマイニング処理などに悪用する。
(これによって、正規のユーザの操作や正規のサービスのパフォーマンスが低下したり、通信が遅延したりする場合もある。) -->
...
<!-- 対応が必要と決定したらCSIRTなどへ報告し、インシデントへの対応要否決定までの詳細な作業記録は報告書へまとめておく。 -->
...
<!-- ここからは、SOCで使用する監視・分析用のセキュリティツールと監視技術について紹介します。 -->
<!-- 
EPP(Endpoint Protection Platform)とは、PCやサーバなどを保護するエンドポイントセキュリティ製品の一種で、マルウェアによる攻撃を水際で防ぐことを目的。主な機能は、ファイルベースのマルウェア攻撃を検知し、調査分析や修復を支援すること。マルウェアの検知方法としては、古くから存在するシグニチャ(定義ファイル)ベースのものに加え、機械学習や振舞い解析といった技術も用いられている。EPPをすり抜けてしまう脅威も存在することから、最近ではEPPに加え、脅威が発生した後の可視化や迅速な対応を可能にするEDRを組み合わせるケースが増えている。 
-->
...
<!-- SOARでは、名前にもある通り、OrchestrationとAutomation、Responseの3つがポイントになります。 -->
...
<!-- SOARの概念図です。 -->
<!-- FWなどからログを収集し、分析や誤検知判定、リスク評価、対応の手順をプレイブックにまとめておきます。これによって、分析から対応までを自動化することができます。 -->
...
<!-- 境界防御方式のネットワークは、FWやIDS/IPSを組織のネットワークの境界に設置して、インターネットなどの外部ネットワークからのサイバー攻撃や侵入を監視する。また、内部ネットワークからの外部ネットワークへの不審な通信も監視する。 -->
